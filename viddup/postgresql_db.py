...
class DB:
    ...
    def make_schema(self) -> None:
        ...
        with self.getconn() as conn:
            with conn.cursor() as c:
                ...
                c.execute(
                    "create unique index if not exists filenames_name_ux on filenames (name)"
                )
                c.execute(
    ...
    def try_lock(self, conn: Connection, fname: str) -> None:
        ...
        with conn.cursor() as c:
            c.execute(
                "delete from locks where time_t < %s", [now - self.params.locktime]
            )
            c.execute(
                "insert into locks (name, hostname, pid, time_t) values (%s, %s, %s, %s)",
                [fname, socket.gethostname(), os.getpid(), time.time()],
            )

    def insert_brightness(
        self, conn: Connection, fid: int, brightness: List[float]
    ) -> None:
        with conn.cursor() as c:
            c.execute("delete from brightness where filename_id = %s", [fid])
            c.execute(
                "insert into brightness (filename_id, brightness) values (%s, %s)",
                [fid, brightness],
            )

    def get_brightness(self, conn: Connection, fid: int) -> List[float]:
        c = conn.execute(
            "select brightness from brightness where filename_id = %s", [fid]
        )
        data = c.fetchone()
        if data:
            return data[0]
    ...
    def get_file_infos(
        self, conn: Connection, fid: Optional[int] = None
    ) -> List[FileInfo]:
        with conn.cursor() as c:
            if fid is None:
                c.execute(
                    "select id, name, fps, duration from filenames order by id asc"
                )
            else:
                c.execute(
                    "select id, name, fps, duration from filenames where id = %s", [fid]
                )
            return [FileInfo._make(i) for i in c.fetchall()]

    def insert_file(
        self, conn: Connection, fname: str, fps: float, duration: float
    ) -> FileInfo:
        fid = self.get_id(conn, fname)
        if fid is None:
            with conn.cursor() as c:
                c.execute(
                    "insert into filenames (name, fps, duration) "
            ...
            with conn.cursor() as c:
                c.execute(
                    "update filenames set name=%s, fps=%s, duration=%s where id=%s",
                    [fname, fps, duration, fid],
                )
        return FileInfo(fid, fname, fps, duration)

    def insert_hashes(
        self, conn: Connection, fid: int, index_info: List[Tuple[int, float]]
    ) -> None:
        with conn.cursor() as c:
            c.execute("delete from hashes where filename_id = %s", [fid])
            for f, h in index_info:
                c.execute(
                    "insert into hashes (filename_id, frame, hash) values (%s, %s, %s)",
                    [fid, f, h],
                )
    ...
    def tidy_db(self, conn: Connection) -> None:
        ...
        with conn.cursor() as c:
            c.execute(
                "delete from filenames where not exists (select 1 from hashes where filename_id = id limit 1)"
            )

    def get_hashes(
        self, conn: Connection, fid: int, min_frame: int, max_frame: int
    ) -> Tuple[List[int], List[float]]:
        with conn.cursor() as c:
            c.execute(
                "select frame, hash from hashes "
