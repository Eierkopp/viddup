...
class DB(DBBase):
    ...
    def get_db(self):
        return pg.connect("host=%s port=%d dbname=%s user=%s password=%s"
                          % (self.params.dbhost, self.params.dbport,
                             self.params.dbname, self.params.dbuser, self.params.dbpwd))
